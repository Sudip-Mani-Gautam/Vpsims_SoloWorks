...
import {
  ...
  Package, MessageSquare, CreditCard, AlertTriangle, Clock
} from "lucide-react";
...
const getTypeStyles = (type?: string) => {
  switch (type) {
    ...
    case "BOOKING_STATUS":
      return { icon: <AlertTriangle className="w-4 h-4" />, color: "text-emerald-500", bg: "bg-emerald-500/10", border: "border-emerald-500/20" };
    default:
...
const AdminNotifications = () => {
  ...
  const handleClick = async (n: AdminNotification) => {
    ...
    if (n.relatedId && (n.type?.includes("SUPPORT"))) {
      navigate(`/admin/support/${n.relatedId}`);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">System & activity alerts</p>
          </div>
        </div>
      ...
      {loading ? (
        <div className="py-32 flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/50">Syncing notifications...</p>
        </div>
      ) : filtered.length === 0 ? (
          ...
          {filtered.map((n, index) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "group relative flex items-start gap-4 px-5 py-4 cursor-pointer transition-all duration-200 hover:bg-muted/40",
